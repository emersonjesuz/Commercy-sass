import { UpdateProductUseCase } from "../../../src/application/usecases/product/UpdateProduct.usecase";
...
describe("Atualizar produto", () => {
  ...
  beforeEach(() => {
    repository = {
      ...
      update: jest
        .fn()
        .mockResolvedValue(new ProductEntity("id-qualquer", "batom", "Batom vermelho da Avon", 1, 10.5, new Date(), "cat-001")),
      findById: jest.fn().mockResolvedValue(new ProductEntity("id-qualquer", "batom", "", 1, 10, new Date(), "cat-001")),
      findAll: jest.fn(),
      delete: jest.fn(),
    };
  ...
  test("Deve não atualizar o produto com nome vazio!", async () => {
    ...
    const productId = "233443443";
    await expect(updateProductUseCase.execute(input, productId)).rejects.toThrow("O nome do produto não pode ser vazio!");
  });
  ...
  test("Deve não atualizar o produto se o catalogId não for informado!", async () => {
    const input = {
      name: "batom",
      description: "Batom vermelho da Avon",
      quantity: 2,
    ...
    const productId = "233443443";
    await expect(updateProductUseCase.execute(input, productId)).rejects.toThrow("O identificador do catalogo é obrigatório!");
  });
});
